...
const { authUser, requireLogin, requireAdmin } = require('../middleware/auth'); //FIX BUG #3
...
router.get('/', authUser, requireLogin, async function(req, res, next) {
  try {
    let usersAll = await User.getAll();
    // FIX BUG #1
    let users = usersAll.map(u => {
...
router.delete('/:username', authUser, requireAdmin, async function(
  ...
  try {
     await User.delete(req.params.username); //FIX BUG #6
