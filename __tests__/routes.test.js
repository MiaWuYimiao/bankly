...
describe("GET /users", function() {
  ...
  //TEST BUG #1
  test("should list basic information", async function() {
    const response = await request(app)
      .get("/users")
      .send({ _token: tokens.u1 });
    expect(response.statusCode).toBe(200);
...
describe("GET /users/[username]", function() {
  ...
  // TEST BUG #2
  test("user cannot be found", async function() {
    const response = await request(app)
    .get("/users/none")
    .send({ _token: tokens.u1 });
...
describe("PATCH /users/[username]", function() {
  ...
  // TEST BUG #3
  test("should patch if it is same user", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1"}); // same user
    expect(response.statusCode).toBe(200);
  });
  ...
  // TEST BUG #4
  test("should disallowed if patching non-existent field", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u3, nonexistentfild: "new-field" }); // u3 is admin
    expect(response.statusCode).toBe(401);
  });

  // TEST BUG #5
  test("should disallowing patching not-allowed-fields: admin", async function() {
    const response = await request(app)
      .patch("/users/u1")
